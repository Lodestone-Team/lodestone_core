...
impl Instance {
    pub async fn new(
        ...
        std::fs::write(&path_to_properties, format!("server-port={}", config.port)).map_err(|_| Error {
            inner: ErrorInner::FailedToWriteFileOrDir,
    ...
    fn read_properties(&mut self) -> Result<(), Error> {
        task::block_in_place(|| {
            *self.settings.blocking_lock() = read_properties_from_path(&self.path_to_properties).map_err(|_| Error { inner: ErrorInner::FailedToReadFileOrDir, detail: "".to_string() })?;
            Ok(())
        })
    }

    fn write_properties_to_file(&self) -> Result<(), Error> {
